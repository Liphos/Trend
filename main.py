...
if __name__ == "__main__":
    ...
    if type(config["dataset"]["extra_args"]["impurity_level"]) == int:
        impurity_levels = [config["dataset"]["extra_args"]["impurity_level"]]
    elif type(config["dataset"]["extra_args"]["impurity_level"]) == list:
        impurity_levels = config["dataset"]["extra_args"]["impurity_level"]
    else:
        raise ValueError("Invalid format for the paramater")
    
    impurity_performance = []
    for impurity_level in impurity_levels:
        config["dataset"]["extra_args"]["impurity_level"] = impurity_level ### THIS SHOULD BE CHANGED !!!
        ...
        if config["training"]["mode"] == "cross_training":
            ...
            impurity_performance.append(1 - (new_nb_correct/len(new_labels)))
            ...
            for folder in os.listdir(f"./Models/{dataset_name}"):
                if os.path.isdir(f"./Models/{dataset_name}/{folder}") and folder.startswith(config["comment"] + '-'):
                    for file in os.listdir(f"./Models/{dataset_name}/{folder}"):
                        if file.startswith("checkpoint0_0") and file.endswith(".pth"):
                            model = config["model"]["name"](**config["model"]["extra_args"]).to(config["device"])
    ...
    if len(impurity_performance) != 0:
        print(impurity_performance)
        plt.plot([impurity_level for impurity_level in impurity_levels], impurity_performance)
        plt.xlabel('impurity at the beginning')
        plt.ylabel("Impurity after 5 iterations")
        plt.title('Relabellisation of the mnist dataset')
        plt.show()
